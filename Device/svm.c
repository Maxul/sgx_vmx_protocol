...
static int svm_mmap(struct file *filp, struct vm_area_struct *vma) {
  ...
  if (remap_pfn_range(vma, vma->vm_start, virt_to_phys(svm_vmem) >> PAGE_SHIFT,
...
static const struct file_operations svm_fops = {
    .owner = THIS_MODULE, .mmap = svm_mmap,
};

static struct miscdevice svm_dev = {
    .minor = SVM_MINOR, .name = SVM_NAME, .fops = &svm_fops, .mode = 0666,
};

...
static int list_view_windows(Display *disp)
{
    ...
    if (!compile_re("Desktop", &name_re) || !compile_re("桌面", &name_re)) {
    ...
    for (i = 4; i < len; i++) {
        if (!do_window_match_name(disp, ((Window *)list)[i], &name_re)) {
            ...
            if (isCounted(((Window *)list)[i]) == false) {
                Node tmp;
                tmp.wid = ((Window *)list)[i];
                tmp.close = 0;
                head.push_back(tmp);
                head_new.push_back(tmp);
            }
        }
    }
...
static void list_pop_windows(Display *dpy, Window w)
{
    ...
    for (i = 0; i < nchildren_return; i++) {
        ...
        if (win_attributes.override_redirect &&
            ...
            pthread_mutex_lock(&mutex_link);
            /* only those that are not in list will be inserted */
            if (isCounted(children_return[i]) == false) {
                Node tmp;
                tmp.wid = children_return[i];
                tmp.close = 0;
                head.push_back(tmp);
                head_new.push_back(tmp);
            }
